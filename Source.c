...
int main() {
	// INIT APP
	Bmi.biWidth = GetSystemMetrics(SM_CXSCREEN) / 3 * 2;
	Bmi.biHeight = GetSystemMetrics(SM_CYSCREEN) / 3 * 2;
	...
	HalfScreen.x = Bmi.biWidth / 2;
	HalfScreen.y = Bmi.biHeight / 2;
...
static WNDPROC WProc(HWND hWnd, UINT Mess, WPARAM w, LPARAM l) {
	switch (Mess) {
		...
		case (WM_MOUSEMOVE):
			if (Menu == GameMenu) {
				XAng += (float)((l & 0x0000FFFF) - (Bmi.biWidth / 2)) * MouseSens;
				YAng += (float)((l >> 16) - (Bmi.biHeight / 2)) * MouseSens;
			} return 0;
